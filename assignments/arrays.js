...
console.log(`Car 33 is a *car_year goes here* *car_make goes here* *car model goes here*` );

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
console.log(inventory.slice(-1)[0]);
...
for (let i = 0; i < inventory.length; i++){
    carList.push(inventory[i].car_year);
}

    for (let i = 0; i < carList.length; i++){
        if(carList[i] < 2000){
            oldCars.push(carList[i]);
        }
    }
console.log("Total cars under year 2000: " + oldCars.length);  
...
for(let i = 0; i < inventory.length; i++){
    carList2.push({ car_make: inventory[i].car_make, car_model: inventory[i].car_model})
}
    for(let i = 0; i < carList2.length; i++) {
        if(carList2[i].car_make == "Audi" || carList2[i].car_make == "BMW") {
            BMWAndAudi.push({car_make: carList2[i].car_make, car_model: carList2[i].car_model});
        }
    }
